...
def category_destinations_famous(request):
    destinations = Destinations.objects.filter(category="Famous").order_by('-Created_date')
    context = {'destinations': destinations}
    return render(request, 'category_destinations.html', context)

@login_required	
def category_destinations_jungle(request):
    destinations = Destinations.objects.filter(category="Jungle").order_by('-Created_date')
    context = {'destinations': destinations}
    return render(request, 'category_destinations.html', context)
